...
sub ask_help {
    ...
    my $request = '<i>Mi dici chiù dâ palora:</i> '. $display .'<i>? Clicca ccà!</i>';
...
sub mk_notex {
	...
	$othtml .= $ccsubs{mk_notex_list}( $typeof , \@prose ) ; 
    }
...
sub mk_cctophtml {
    ...
    $ottxt .= 'Chiù dâ Palora :: Napizia</title>' ."\n";

    if ( $descrip_fmt ne "" ) { 
	$ottxt .= '    <meta name="DESCRIPTION" content="Chiù dâ palora: ';
...
sub mk_ddtophtml {
    ...
    $ottxt .= '<html>' . "\n" ;
    $ottxt .= '  <head>' . "\n" ;
    $ottxt .= '    <title>' . $title_insert  . 'Dizziunariu Dieli :: Napizia</title>'."\n";
